...
func init() {
	var err error
	ffmpegPath, err = writeTempExec("ffmpeg_linux_386", ffmpeg)
	if err != nil {
		panic(fmt.Errorf("failed to write ffmpeg_linux_386: %v", err))
	}
	ffprobePath, err = writeTempExec("ffprobe_linux_386", ffprobe)
	if err != nil {
		panic(fmt.Errorf("failed to write ffprobe_linux_386: %v", err))
	}
}
